...
-- ----------------------------
INSERT INTO `estadisticas_defensivas` VALUES (1, 1, 1, 9.00, 2, 2, 0, 0, 2.0, 0, 0, 0, 0);
...
-- ----------------------------
INSERT INTO `estadisticas_equipos` VALUES (1, 1, 2, 3, 400.00, 5, 1);
INSERT INTO `estadisticas_equipos` VALUES (2, 2, 3, 2, 600.00, 5, 1);
...
-- ----------------------------
INSERT INTO `estadisticas_ofensivas` VALUES (1, 1, 2, 500, 0, 5, 3, 0, 1, 2, 1, 1, 10, 0);
INSERT INTO `estadisticas_ofensivas` VALUES (2, 1, 3, 625, 2, 10, 2, 0, 2, 1, 0, 2, 8, 0);
...
INSERT INTO `jugadas` VALUES (5, 0, 'cs');
...
INSERT INTO `jugadas` VALUES (13, 0, 'ce');
INSERT INTO `jugadas` VALUES (14, 0, 'ca');
...
-- ----------------------------
-- Triggers structure for table juego_detalle
-- ----------------------------
DROP TRIGGER IF EXISTS `jugada`;
delimiter ;;
CREATE DEFINER = `id17622284_graco`@`localhost` TRIGGER `jugada` BEFORE INSERT ON `juego_detalle` FOR EACH ROW BEGIN
	CASE NEW.jugada
	
	WHEN 1 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.k = unidos.estadisticas_ofensivas.k + 1, estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
	
		WHEN 2 THEN
	UPDATE unidos.estadisticas_ofensivas set  estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
	...
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
		...
		WHEN 6 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.sb = unidos.estadisticas_ofensivas.sb + NEW.br
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
	
		WHEN 7 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.bb = unidos.estadisticas_ofensivas.bb + 1
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
	
	-- ESTADISTICAS HITS
	
	WHEN 9 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.h = unidos.estadisticas_ofensivas.h + 1, estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1,unidos.estadisticas_ofensivas.ce = unidos.estadisticas_ofensivas.ce + NEW.ce
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
	
	WHEN 10 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.h2 = unidos.estadisticas_ofensivas.h2 + 1, estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1,unidos.estadisticas_ofensivas.ce = unidos.estadisticas_ofensivas.ce + NEW.ce
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
	WHEN 11 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.h3 = unidos.estadisticas_ofensivas.h3 + 1, estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1,unidos.estadisticas_ofensivas.ce = unidos.estadisticas_ofensivas.ce + NEW.ce
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
	
	WHEN 12 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.h4 = unidos.estadisticas_ofensivas.h4 + 1, estadisticas_ofensivas.vb =  estadisticas_ofensivas.vb + 1,unidos.estadisticas_ofensivas.ce = unidos.estadisticas_ofensivas.ce + NEW.ce,unidos.estadisticas_ofensivas.ca = unidos.estadisticas_ofensivas.ca + NEW.ca
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
	
	-- ESTDISTICAS HITS
	
		WHEN 13 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.ce = unidos.estadisticas_ofensivas.ce + NEW.ce
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
	
		WHEN 14 THEN
	UPDATE unidos.estadisticas_ofensivas set unidos.estadisticas_ofensivas.ca = unidos.estadisticas_ofensivas.ca + NEW.ca
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;
...
-- Triggers structure for table juego_detalle
-- ----------------------------
DROP TRIGGER IF EXISTS `estadisticas_defensivas`;
delimiter ;;
CREATE DEFINER = `root`@`localhost` TRIGGER `estadisticas_defensivas` BEFORE INSERT ON `juego_detalle` FOR EACH ROW BEGIN
	CASE NEW.jugada
	
	WHEN 1 THEN
	UPDATE unidos.estadisticas_defensivas set unidos.estadisticas_defensivas.k = unidos.estadisticas_defensivas.k + 1, estadisticas_defensivas.entradas = ( estadisticas_defensivas.entradas + (1 / 3))
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;
	
	WHEN 2 THEN
	UPDATE unidos.estadisticas_defensivas set  estadisticas_defensivas.entradas = ( estadisticas_defensivas.entradas + (1 / 3))
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	
	
	WHEN 3 THEN
	UPDATE unidos.estadisticas_defensivas set  estadisticas_defensivas.entradas = ( estadisticas_defensivas.entradas + ( 2 / 3))
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	

	WHEN 4 THEN
	UPDATE unidos.estadisticas_defensivas set  estadisticas_defensivas.entradas = ( estadisticas_defensivas.entradas + (3 / 3))
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	

	WHEN 7 THEN
	UPDATE unidos.estadisticas_defensivas set  estadisticas_defensivas.bb = estadisticas_defensivas.bb + 1
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	

	WHEN 8 THEN
	UPDATE unidos.estadisticas_defensivas set  estadisticas_defensivas.wp = estadisticas_defensivas.wp + 1
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	

			WHEN 5 THEN
	UPDATE unidos.estadisticas_defensivas set unidos.estadisticas_defensivas.cp = unidos.estadisticas_defensivas.cp + NEW.ce
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;
...
-- Triggers structure for table juego_detalle
-- ----------------------------
DROP TRIGGER IF EXISTS `estadisticas_ofensivas_turnos`;
delimiter ;;
CREATE DEFINER = `id17622284_graco`@`localhost` TRIGGER `estadisticas_ofensivas_turnos` AFTER INSERT ON `juego_detalle` FOR EACH ROW BEGIN

	UPDATE unidos.estadisticas_ofensivas 
	set unidos.estadisticas_ofensivas.avg = ((h+h2+h3+h4) / vb) * 1000
	WHERE unidos.estadisticas_ofensivas.id_jugador =  NEW.bateador;	
...
-- Triggers structure for table juego_detalle
-- ----------------------------
DROP TRIGGER IF EXISTS `efectividad`;
delimiter ;;
CREATE DEFINER = `id17622284_graco`@`localhost` TRIGGER `efectividad` AFTER INSERT ON `juego_detalle` FOR EACH ROW BEGIN

	UPDATE unidos.estadisticas_defensivas 
	set unidos.estadisticas_defensivas.efectividad = (estadisticas_defensivas.cp * 9) / estadisticas_defensivas.entradas
	WHERE unidos.estadisticas_defensivas.id_jugador =  NEW.picher;	
...
delimiter ;;
CREATE DEFINER = `id17622284_graco`@`localhost` TRIGGER `estadisticas` BEFORE INSERT ON `juegos` FOR EACH ROW BEGIN
...
delimiter ;;
CREATE DEFINER = `id17622284_graco`@`localhost` TRIGGER `estadisticas2` AFTER INSERT ON `juegos` FOR EACH ROW BEGIN
	 update unidos.estadisticas_equipos 
